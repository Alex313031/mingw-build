...
[llvm-support-pre-vista.patch](./llvm/llvm-support-pre-vista.patch) - Makes the Windows-hosted LLVM **toolchain binaries** (`clang.exe`, `lld`, `llvm-*`) actually LOAD and run on Windows XP/2000. Unlike the other LLVM patches (which fix the *runtime libraries* used by compiled programs), this patches LLVM's own `lib/Support` (`Windows/Path.inc` + `Windows/Threading.inc`), which statically imports six Vista+/Win7+ functions
                                                                       ([GetFinalPathNameByHandleW](https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-getfinalpathnamebyhandlew), [SetFileInformationByHandle](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-setfileinformationbyhandle), and the processor-group APIs `GetLogicalProcessorInformationEx` / `Get`/`SetThreadGroupAffinity` / `GetProcessGroupAffinity`). A static import of a function the OS lacks makes the whole `.exe` fail to start, so each is resolved at runtime via `GetProcAddress` (the same trick LLVM already uses for `PrefetchVirtualMemory`/`SetThreadInformation`). When absent: `realPathFromHandle` fails soft, `rename()` falls back to [MoveFileExW](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-movefileexw) by path, and the processor-group code collapses to a single `GetSystemInfo` group with no thread-group affinity.
