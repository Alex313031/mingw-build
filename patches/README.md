...
[llvm-support-pre-vista.patch](./llvm/llvm-support-pre-vista.patch) - Makes the Windows-hosted LLVM **toolchain binaries** (`clang.exe`, `lld`, `llvm-*`) actually LOAD and run on Windows XP/2000. Unlike the other LLVM patches (which fix the *runtime libraries* used by compiled programs), this patches LLVM's own `lib/Support` (`Windows/Path.inc` + `Windows/Threading.inc` + `Windows/Memory.inc`), which statically imports Vista+/Win7+ functions
                                                                       (`Path.inc`: [GetFinalPathNameByHandleW](https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-getfinalpathnamebyhandlew), [SetFileInformationByHandle](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-setfileinformationbyhandle), [SHGetKnownFolderPath](https://learn.microsoft.com/en-us/windows/win32/api/shlobj_core/nf-shlobj_core-shgetknownfolderpath), [SHCreateItemFromIDList](https://learn.microsoft.com/en-us/windows/win32/api/shobjidl_core/nf-shobjidl_core-shcreateitemfromidlist); `Threading.inc`: the processor-group APIs `GetLogicalProcessorInformationEx` / `Get`/`SetThreadGroupAffinity` / `GetProcessGroupAffinity`; `Memory.inc`: `GetLargePageMinimum`). A static import of a function the OS lacks makes the whole `.exe` fail to start, so each is resolved at runtime via `GetProcAddress` (the same trick LLVM already uses for `PrefetchVirtualMemory`/`SetThreadInformation`). When absent: `realPathFromHandle` fails soft, `rename()` falls back to [MoveFileExW](https://learn.microsoft.com/en-us/windows/win32/api/winbase/nf-winbase-movefileexw) by path, `SHGetKnownFolderPath` falls back to `SHGetFolderPathW`+CSIDL, `remove_directories` falls back to the classic `SHFileOperationW`, the processor-group code collapses to a single `GetSystemInfo` group, and `GetLargePageMinimum` returns 0 (large pages disabled). Separately, the psapi forwarders `K32EnumProcessModules`/`K32GetProcessMemoryInfo` are handled by building with `-DPSAPI_VERSION=1` (routes them to `psapi.dll`), set in `mingw_llvm_win.sh`, not by this patch.
