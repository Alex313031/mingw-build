...
apply_patches() {
  ...
  # time (so the clang/lld cmake below passes -D_WIN32_WINNT=$WIN32_WINNT). Also
...
EOF
  ...
  # clang-cl: the MSVC-syntax driver, retargeted at this toolchain's mingw sysroot.
  # Stock clang-cl forces a *-pc-windows-msvc target and expects the Microsoft SDK
  # + MSVC STL (which this toolchain does not ship); pinning --target flips it to
  # the GNU/mingw environment so cl.exe-style flags (/O2 /EHsc /std:c++17 /W4 ...)
  # compile against the bundled libc++/mingw runtime instead. Notes:
  #   - clang-cl ignores a bare --sysroot, so it rides in via the /clang: escape.
  #   - /Zl drops the MSVC default-lib directives (libcmt/oldnames) clang-cl bakes
  #     into every .obj; without it the mingw linker can't resolve them.
  # This is MSVC *syntax*, not the MSVC *ABI* (name-mangling/layout stay Itanium).
  # Compile here (/c), then link with $wrap-clang++ (add -pthread for libc++ code
  # that uses threads or thread-safe statics).
  cat > "$bindir/$wrap-clang-cl" <<EOF
#!/bin/sh
# Auto-generated by $SCRIPTNAME for $wrap
dir=\$(CDPATH= cd -- "\$(dirname -- "\$0")" && pwd)
exec "\$dir/clang" --driver-mode=cl --target=$triple /clang:--sysroot="\$dir/../$triple" /Zl "\$@"
EOF
  chmod +x "$bindir/$wrap-clang-cl"
...
build() {
  ...
  # --incremental keeps the build tree + prefix so the build resumes where it left
  # off; only wipe them on a normal build.
  if [ ! "$CLANG_FORMAT_ONLY" ] && [ ! "$INCREMENTAL" ]; then
...
USE_AVX512=$avx512"
  ...
  # (~70MB) -- is left uninstalled, taking the toolchain from ~2GB to a few
  # hundred MB. The runtimes/compiler-rt are installed separately (own prefix
  # passes) so they are unaffected. (clang++/clang-cl ride with the clang
  # component; llvm-ranlib/llvm-lib/llvm-dlltool/llvm-strip/llvm-addr2line are
  # symlinks listed by their own name.)
  #
  ...
  # Build only the distribution set (what install-distribution ships) rather than
  # everything: clang-tools-extra is enabled for clang-tidy, but this keeps the
  # rest of it -- clangd especially -- from building just to be discarded.
  ...
  # SIMD baseline + exception model: read them from the toolchain compiler's own
  # predefined macros (this arch's baseline flags, via the target-cc wrapper) so
  # they are right even here, where mingw-ver is built by the host's system c++.
  _mv_pd=$(printf '' | "$prefix/bin/$triple-cc" $SIMD_FLAGS ${MARCH:+-march=$MARCH} -dM -E - 2>/dev/null)
...
install_deps() {
  ...
  # build-essential's g++ is sufficient. libxml2-dev is required by llvm-mt (the
  # manifest tool), which we ship in this Linux-hosted toolchain -- see the
  # LLVM_ENABLE_LIBXML2/llvm_dist notes in build().
