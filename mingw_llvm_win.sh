...
# carries the target triple and the sysroot (relative to the config dir via
# <CFGDIR>, so the prefix can live anywhere on the target) plus the link-time PE
# subsystem/OS version. Deliberately NO -march/SIMD flags (build-time only) -- so
# users can pass their own microarchitecture flags without the wrapper fighting.
# $1 = arch (wrapper name), $2 = clang target triple, $3 = prefix
# Globals used: SUBSYS_LDFLAGS
