...
# we still bake --target, --sysroot and the arch SIMD flags into wrappers so the
# toolchain defaults to this arch even when invoked with no extra flags (this is
# how i586 stays no-SSE by default while sharing clang's i686 triple).
# $1 = arch (wrapper name), $2 = clang target triple, $3 = prefix, $4 = SIMD flags
generate_wrappers() {
  local arch="$1" triple="$2" prefix="$3" simd="$4" ldextra="$5"
  local wrap="$arch-w64-mingw32"
  local bindir="$prefix/bin"
  log "${GRE}Generating toolchain wrappers for ${bold}$wrap${c0}${GRE} (target $triple)${c0}\n"

  # $ldextra carries link-time defaults (PE subsystem/OS version). They are
...
exec "$bindir/clang" $mode --target=$triple --sysroot="$prefix/$triple" $simd $ldextra -Wno-unused-command-line-argument "\$@"
EOF
...
# clang.exe infers its target from the argv[0] triple prefix and its driver
# mode (C vs C++) from the "++"/"g++"/"c++" in the name, so the driver entry
# points are just hardlinks to clang.exe; the binutils-style tools hardlink to
# the matching llvm-*.exe. A single auto-loaded clang config file ($wrap.cfg)
# carries the real target triple (normalizing i586 -> i686), the sysroot
# (relative to the config dir, so the prefix can live anywhere on the target),
# the arch SIMD flags and the PE subsystem/OS version.
# $1 = arch (wrapper name), $2 = clang target triple, $3 = prefix
# Globals used: SIMD_FLAGS, SUBSYS_LDFLAGS
generate_wrappers_windows() {
  local arch="$1" triple="$2" prefix="$3"
  local wrap="$arch-w64-mingw32"
  local bindir="$prefix/bin"
  log "${GRE}Generating Windows toolchain wrappers for ${bold}$wrap${c0}${GRE} (target $triple)${c0}\n"

  # Auto-loaded for any "$wrap-*" clang invocation. <CFGDIR> resolves to this
  # file's directory at runtime, keeping the toolchain relocatable.
  cat > "$bindir/$wrap.cfg" <<EOF
# Auto-generated by $SCRIPTNAME for $wrap
--target=$triple
--sysroot=<CFGDIR>/../$triple
$SIMD_FLAGS
$SUBSYS_LDFLAGS
-Wno-unused-command-line-argument
EOF

  # Driver entry points -> clang.exe (target + C/C++ mode inferred from name).
  local name
  for name in clang clang++ gcc g++ cc c++; do
    execute "" "Failed to link $wrap-$name.exe" \
        cp -lf "$bindir/clang.exe" "$bindir/$wrap-$name.exe"
  done

  # binutils-style tools -> the matching llvm-*.exe.
  local tool
  for tool in ar:llvm-ar ranlib:llvm-ranlib nm:llvm-nm strip:llvm-strip \
              objcopy:llvm-objcopy objdump:llvm-objdump dlltool:llvm-dlltool \
              windres:llvm-windres strings:llvm-strings addr2line:llvm-addr2line \
              ld:ld.lld; do
    execute "" "Failed to link $wrap-${tool%%:*}.exe" \
        cp -lf "$bindir/${tool#*:}.exe" "$bindir/$wrap-${tool%%:*}.exe"
  done
}
...
compute_arch_flags() {
  ...
  # clang has no i586 triple: i586 and i686 both build for i686-w64-mingw32 and
  # differ only in the -march/SIMD flags baked into the runtimes and wrappers.
  if [ "$arch" = "x86_64" ]; then
...
USE_AVX512=$avx512"
  ...
  AUTOTOOLS_CFLAGS="$OPT_FLAGS -pipe"
...
build_phase1_linux() {
  ...
  generate_wrappers "$arch" "$triple" "$prefix" "$SIMD_FLAGS" "$SUBSYS_LDFLAGS"
...
build_phase2_windows() {
  ...
  execute "($arch P2): Configuring Windows-hosted LLVM" "Configuring Windows LLVM failed" \
      ...
      -DCMAKE_C_FLAGS="$TARGET_CFLAGS" \
      -DCMAKE_CXX_FLAGS="$TARGET_CXXFLAGS" \
      -DCMAKE_EXE_LINKER_FLAGS="-static $SUBSYS_LDFLAGS $STRIP_FLAG" \
  ...
  generate_wrappers_windows "$arch" "$triple" "$prefix"
